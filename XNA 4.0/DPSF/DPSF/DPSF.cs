...
namespace DPSF
{
...
#endif
    {
        ...
        protected static string[] CreateTextureArrayFromBaseName(string baseName, int textureCount, int startIndex)
        {
            string[] particleTextures = new string[textureCount];
            for (int i = 0; i < textureCount; ++i)
            {
                particleTextures[i] = baseName + startIndex++;
            }
            return particleTextures;
        }
        protected void UpdateParticleTextures(ref float elapsedTime, ref int textureIndex, float textureSwapDelay, in string[] textures, bool repeat, DPSFParticle cParticle, float fElapsedTimeInSeconds)
        {
            elapsedTime += fElapsedTimeInSeconds;

            if (elapsedTime >= textureSwapDelay)
            {
                elapsedTime = 0f;

                if (!IncreaseTexture(ref textureIndex, in textures))
                {
                    if (repeat)
                    {
                        textureIndex = 0;
        ...
        bool IncreaseTexture(ref int textureIndex, in string[] textures)
        {
        ...
        public class CParticleEvents
        {
            ...
            [Serializable]
#endif
            class CParticleEvent(DPSF<Particle, Vertex>.UpdateParticleDelegate _cFunctionToCall, CParticleEvents.EParticleEventTypes _eType, int _iExecutionOrder, int _iGroup)
            {
                public UpdateParticleDelegate cFunctionToCall = _cFunctionToCall;  // The Function to call when the Event fires
                public EParticleEventTypes eType = _eType;               // The Type of Event this is
                public int iExecutionOrder = _iExecutionOrder;                     // The Order, relative to other Events, of when this Event should Execute (i.e. Fire)
                public int iGroup = _iGroup;                              // The Group that this Event belongs to
                ...
                public static bool operator ==(CParticleEvent c1, CParticleEvent c2)
                    => (c1.cFunctionToCall == c2.cFunctionToCall &&
            ...
            [Serializable]
#endif
            class CTimedParticleEvent(DPSF<Particle, Vertex>.UpdateParticleDelegate _cFunctionToCall, CParticleEvents.EParticleEventTypes _eTimedType,
                                       int _iExecutionOrder, int _iGroup, float _fTimeToFire) : CParticleEvent(_cFunctionToCall, _eTimedType, _iExecutionOrder, _iGroup)
            {
                public float fTimeToFire = _fTimeToFire;   // The Time at which this Event should Fire
                ...
                public override bool Equals(object obj)
                {
                    if (obj is not CTimedParticleEvent)
                    {
                        return false;
                    }
            ...
            private int EventSorter(CParticleEvent c1, CParticleEvent c2)
                => c1.iExecutionOrder.CompareTo(c2.iExecutionOrder);

            private int EventSorterAsync(CParticleEvent c1, CParticleEvent c2)
            {
                if (c1 == null)
                {
                    return 1;
            ...
            private readonly List<CParticleEvent> mcParticleEventList = [];
            ...
            public void AddEveryTimeEvent(UpdateParticleDelegate cFunctionToCall, int iExecutionOrder, int iGroup)
            {
                ...
                // WCS Multithread safety edit.

                // Instead only this line.
                mcParticleEventList.Sort(EventSorter);

                // Use this method !
                // SortEventList(mcParticleEventList, EventSorterAsync);
            }

            static System.Timers.Timer sortEventListTimer;
            static void SortEventList(List<CParticleEvent> mcParticleEventList, Comparison<CParticleEvent> eventSorter)
            {
                if (sortEventListTimer == null)
                {
                    const int retryInterval = 1000;
                    sortEventListTimer = new(retryInterval);
                    sortEventListTimer.Elapsed += (s, e) => SortEventList(mcParticleEventList, eventSorter);
                }
                sortEventListTimer.Stop();
                try
                {
                    mcParticleEventList.Sort(eventSorter);
                }
            ...
            public bool RemoveEveryTimeEvent(UpdateParticleDelegate cFunction, int iExecutionOrder, int iGroup)
                ...
                => mcParticleEventList.Remove(new CParticleEvent(cFunction, EParticleEventTypes.EveryTime, iExecutionOrder, iGroup));
            ...
            public bool RemoveOneTimeEvent(UpdateParticleDelegate cFunction, int iExecutionOrder, int iGroup)
                ...
                => mcParticleEventList.Remove(new CParticleEvent(cFunction, EParticleEventTypes.OneTime, iExecutionOrder, iGroup));
            ...
            public bool ContainsOneTimeEvent(UpdateParticleDelegate cFunction, int iExecutionOrder, int iGroup)
                ...
                => mcParticleEventList.Contains(new CParticleEvent(cFunction, EParticleEventTypes.OneTime, iExecutionOrder, iGroup));
        ...
        [Serializable]
#endif
        public sealed class CParticleSystemEvents
        {
            /// <summary>
            ...
            [Serializable]
#endif
            class CParticleSystemEvent(UpdateParticleSystemDelegate _cFunctionToCall, EParticleSystemEventTypes _eType, int _iExecutionOrder, int _iGroup)
            {
                public UpdateParticleSystemDelegate cFunctionToCall = _cFunctionToCall;    // The Function to call when the Event fires
                public EParticleSystemEventTypes eType = _eType;                 // The Type of Event this is
                public int iExecutionOrder = _iExecutionOrder;                             // The Order, relative to other Events, of when this Event should Execute (i.e. Fire)
                public int iGroup = _iGroup;                                      // The Group the Event belongs to
                ...
                public override bool Equals(object obj)
                {
                    if (obj is not CParticleSystemEvent)
                    {
                        return false;
                    }
            ...
            [Serializable]
#endif
            sealed class CTimedParticleSystemEvent
                (UpdateParticleSystemDelegate _cFunctionToCall, EParticleSystemEventTypes _eTimedType,
                 int _iExecutionOrder, int _iGroup, float _fTimeToFire)
                 : CParticleSystemEvent(_cFunctionToCall, _eTimedType, _iExecutionOrder, _iGroup)
            {
                public float fTimeToFire = _fTimeToFire;   // The Time at which this Event should Fire
            ...
            // List to hold all of the Events
            private readonly List<CParticleSystemEvent> mcParticleSystemEventList = [];

            // Variable to hold the Lifetime Data of the Particle System
            private CParticleSystemLifetimeData mcParticleSystemLifetimeData = new();
            ...
            {
                get => mcParticleSystemLifetimeData;
            ...
            public bool RemoveEveryTimeEvent(UpdateParticleSystemDelegate cFunction, int iExecutionOrder, int iGroup)
                // {
                // Remove the specified Event
                => mcParticleSystemEventList.Remove(new CParticleSystemEvent(cFunction, EParticleSystemEventTypes.EveryTime, iExecutionOrder, iGroup));
            // }
            ...
            public bool ContainsEveryTimeEvent(UpdateParticleSystemDelegate cFunction, int iExecutionOrder, int iGroup)
                // {
                // Return if there is an EveryTime Event with the specified criteria or not
                => mcParticleSystemEventList.Contains(new CParticleSystemEvent(cFunction, EParticleSystemEventTypes.EveryTime, iExecutionOrder, iGroup));
            // }
            ...
            public bool RemoveOneTimeEvent(UpdateParticleSystemDelegate cFunction, int iExecutionOrder, int iGroup)
                // {
                // Remove the specified Event from the Event List
                => mcParticleSystemEventList.Remove(new CParticleSystemEvent(cFunction, EParticleSystemEventTypes.OneTime, iExecutionOrder, iGroup));
            // }
            ...
            public bool ContainsOneTimeEvent(UpdateParticleSystemDelegate cFunction, int iExecutionOrder, int iGroup)
                // {
                // Return if there is an OneTime Event with the specified criteria or not
                => mcParticleSystemEventList.Contains(new CParticleSystemEvent(cFunction, EParticleSystemEventTypes.OneTime, iExecutionOrder, iGroup));
            // }
            ...
            public bool RemoveTimedEvent(float fTimeToFire, UpdateParticleSystemDelegate cFunction, int iExecutionOrder, int iGroup)
                // {
                // Remove the Timed Event from the Event List
                => mcParticleSystemEventList.Remove(new CTimedParticleSystemEvent(cFunction, EParticleSystemEventTypes.Timed, iExecutionOrder, iGroup, fTimeToFire));
            // }
            ...
            public bool ContainsTimedEvent(float fTimeToFire, UpdateParticleSystemDelegate cFunction, int iExecutionOrder, int iGroup)
                // {
                // Return if there is a Timed Event with the specified criteria or not
                => mcParticleSystemEventList.Contains(new CTimedParticleSystemEvent(cFunction, EParticleSystemEventTypes.Timed, iExecutionOrder, iGroup, fTimeToFire));
            // }
            ...
            public int RemoveNormalizedTimedEvents(UpdateParticleSystemDelegate cFunction)
                // {
                // Remove all Normalized Timed Events with the specified Function
                => mcParticleSystemEventList.RemoveAll(delegate (CParticleSystemEvent cEvent)
            ...
            public bool RemoveNormalizedTimedEvent(float fNormalizedTimeToFire, UpdateParticleSystemDelegate cFunction, int iExectionOrder, int iGroup)
                // {
                // Remove the Normalized Timed Event from the Event List
                => mcParticleSystemEventList.Remove(new CTimedParticleSystemEvent(cFunction, EParticleSystemEventTypes.NormalizedTimed, iExectionOrder, iGroup, fNormalizedTimeToFire));
            // }
            ...
            public bool ContainsNormalizedTimedEvent(UpdateParticleSystemDelegate cFunction)
                // {
                // Return if there is a NormalizedTimed Event with the specified Function
                => mcParticleSystemEventList.Exists(delegate (CParticleSystemEvent cEvent)
                { return (cEvent.eType == EParticleSystemEventTypes.NormalizedTimed && cEvent.cFunctionToCall == cFunction); });
            // }
            ...
            public bool ContainsNormalizedTimedEvent(float fNormalizedTimeToFire, UpdateParticleSystemDelegate cFunction, int iExecutionOrder, int iGroup)
                // {
                // Return if there is a NormalizedTimed Event with the specified criteria or not
                => mcParticleSystemEventList.Contains(new CTimedParticleSystemEvent(cFunction, EParticleSystemEventTypes.NormalizedTimed, iExecutionOrder, iGroup, fNormalizedTimeToFire));
            // }
            ...
            public int RemoveAllTimedAndNormalizedTimedEvents()
            // {
                => RemoveAllTimedEvents() + RemoveAllNormalizedTimedEvents();
            // }
        ...
        protected InitializeParticleDelegate mcParticleInitializationFunction;
        ...
        private Particle[] mcParticles;      // Array used to hold all Particles
        private int miNumberOfParticlesToDraw;  // Tells how many Particles in the Vertex Buffer should be drawn (i.e. How many are Active and Visible)

        private Vertex[] mcParticleVerticesToDraw;   // Array used to hold Particle Vertex information (i.e. The Vertex Buffer used to draw the Point Sprites and Quads)
        private int[] miaIndexBufferArray;           // The 32-bit Index Buffer used along with the Vertex Buffer for drawing Quads in the HiDef profile
        private short[] msaIndexBufferReachArray;    // The 16-bit Index Buffer used along with the Vertex Buffer for drawing Quads in the Reach profile
        private int miIndexBufferIndex;                 // The current Index we are at in the Index Buffer
        private static bool? _SuseIntIndexBuffer;    // Tells whether the Int or Short Index Buffer should be used.
#if (WINDOWS)
        [NonSerialized]
#endif
        private SpriteBatch mcSpriteBatch;                   // The Sprite Batch used to draw Sprites
        private Particle[] mcParticleSpritesToDraw;          // The List of Sprites to Draw (i.e. The Vertex Buffer used to draw Sprites)
        private SpriteBatchSettings mcSpriteBatchSettings;   // Variable used to hold the Sprite Batch's drawing options

        private LinkedList<Particle> mcActiveParticlesList;      // List used to hold Active (alive) Particles
        private LinkedList<Particle> mcInactiveParticlesList;    // List used to hold Inactive (dead) Particles

#if (WINDOWS)
        [NonSerialized]
#endif
        private VertexDeclaration mcVertexDeclaration;   // Description of the Vertex Structure used to draw Particles
#if (XBOX)
        private int miVertexSizeInBytes;    // The Size of a Vertex in Bytes
#endif
        ...
        [NonSerialized]
#endif
        private Effect mcEffect;         // The Effect file to use to draw the Particles
#if (WINDOWS)
        [NonSerialized]
#endif
        private Texture2D mcTexture;     // The Texture used to draw Particles

        private int miMaxNumberOfParticlesAllowed;  // The Max Number of Particles that this Particle System is Allowed to contain at a given time
        ...
        private int miDrawOrder;                // Tells where in the Order this particle system should be Drawn
        private int miUpdateOrder;              // Tells where in the Order this particle system should be Updated

#if (WINDOWS)
        [NonSerialized]
#endif
        private Game mcGame;                 // The Game object passed into the constructor (Not used by DPSF at all; Just here for consistency with DrawableGameComponent)

#if (WINDOWS)
        [NonSerialized]
#endif
        private GraphicsDevice mcGraphicsDevice; // Handle to the Graphics Device to draw to
        ...
        [NonSerialized]
#endif
        private ContentManager mcContentManager; // Handle to the Content Manager used to load Textures and Effects
        ...
        private float mfTimeToWaitBetweenUpdates;    // How much time should elapse between Update()s
        private float mfTimeElapsedSinceLastUpdate;  // How much time has elapsed since the last Update()

        private CParticleEvents mcParticleEvents; // Variable used to access the Particle Events
        private CParticleSystemEvents mcParticleSystemEvents = new();// Variable used to access the Particle System Events

        // Auto Memory Manager variables
        private AutoMemoryManagerSettings mcAutoMemoryManagerSettings;  // Auto Memory Manager Settings
        private float mfAutoMemoryManagersElapsedTime;                   // The Automatic Memory Manager's Timer
        ...
        protected ParticleEmitter _emitter; // The Emitter used to automatically generate Particles
        private ParticleEmitterCollection _emitters = new(); // The collection of emitters that this particle system uses.
        ...
        [NonSerialized]
#endif
        private Stopwatch _performanceProfilingStopwatch; // Stopwatch used for diagnostic timers.
#endif
        private bool _performanceProfilingIsEnabled;

        private int miID;                       // Set this Particle Systems unique ID
        private int miType;                     // The Type of Particle System this is
        ...
        private ParticleSystemManager mcParticleSystemManager;   // The Manager whose properties should be used for this Particle System

        private static DPSFDefaultEffect SmcDPSFEffect;          // Handle to the default DPSF Effect
		...
		private int miMaxParticlesThatXboxCanDrawAtOnce;				// Will hold the max number of particles that the Xbox 360 is able to draw in a single Draw() call.
#endif
        ...
        public bool InitializeSpriteParticleSystem(
            GraphicsDevice cGraphicsDevice, ContentManager cContentManager,
            int iNumberOfParticlesToAllocateMemoryFor, int iMaxNumberOfParticlesToAllow,
            string sTexture, SpriteBatch cSpriteBatchToDrawWith)
        {
            try
            {
                // Initialize the variables that all Particle Systems have in common
            ...
            catch // (Exception e)
            {
                // Specify that the Particle System is not yet Initialized and re-throw the exception
                // ParticleType = ParticleTypes.None;
                // Do not throw (WCS edit.) Naahh Throw it anyway ...
                // Console.WriteLine("A problem occurred while Initializing the Particle System. Inner DPSFException: " + e.ToString());
                // throw new DPSFException("A problem occurred while Initializing the Particle System. Inner DPSFException: " + e.ToString(), e);
                return false;
            }
        ...
        /// <para>If not null, then you must call SpriteBatch.Begin() before calling ParticleSystem.Draw() to
        /// draw the particle system, and then call SpriteBatch.End() when done drawing the particle system.</para></param>
        public bool InitializeSpriteParticleSystem(GraphicsDevice cGraphicsDevice, ContentManager cContentManager,
        ...
        public bool InitializeQuadParticleSystem(GraphicsDevice cGraphicsDevice, ContentManager cContentManager,
                                int iNumberOfParticlesToAllocateMemoryFor, int iMaxNumberOfParticlesToAllow,
        ...
        /// <param name="cVertexUpdateFunction">Function used to copy a Particle's drawable properties into the vertex buffer</param>
        /// <param name="sTexture">The asset name of the Texture to use to visualize the Particles</param>
        public bool InitializeTexturedQuadParticleSystem(GraphicsDevice cGraphicsDevice, ContentManager cContentManager,
                                int iNumberOfParticlesToAllocateMemoryFor, int iMaxNumberOfParticlesToAllow,
        ...
        public bool IsInitialized
            => (mcParticles != null && meParticleType != ParticleTypes.None && (this.GraphicsDevice != null || meParticleType == ParticleTypes.NoDisplay));
        ...
        public CParticleEvents GetParticleEvents()
            => ParticleEvents;
        public CParticleSystemEvents GetParticleSystemEvents()
        ...
        public ParticleEmitter Emitter
        {
            ...
            set
            {
                _emitter = value ?? throw new DPSFArgumentNullException("Emitter", "An invalid Emitter was specified. The Emitter cannot be null.");
        ...
        public Matrix WorldViewProjection
            => Matrix.Multiply(Matrix.Multiply(this.World, this.View), this.Projection);
        ...
        {
            get => _performanceProfilingIsEnabled;
            set
            {
        ...
        /// </summary>
        public virtual int TotalNumberOfParticlesAllocatedInMemory => NumberOfParticlesAllocatedInMemory;
        ...
        {
            get => miMaxNumberOfParticlesAllowed;
            set
            {
        ...
        /// </summary>
        public virtual int TotalNumberOfActiveParticles => NumberOfActiveParticles;
        ...
        /// </summary>
        public virtual int TotalNumberOfParticlesBeingDrawn => NumberOfParticlesBeingDrawn;
        ...
        {
            get => ParticleSystemEvents.LifetimeData.EndOfLifeOption == CParticleSystemEvents.EParticleSystemEndOfLifeOptions.Repeat;
            set => ParticleSystemEvents.LifetimeData.EndOfLifeOption = !value ? CParticleSystemEvents.EParticleSystemEndOfLifeOptions.Destroy : CParticleSystemEvents.EParticleSystemEndOfLifeOptions.Repeat;
        }

        public float LifeTime
        {
            get => ParticleSystemEvents.LifetimeData.Lifetime;
        ...
        public bool AddParticle(Particle cParticleToCopy)
        {
            try
            {
                ...
                if (cParticleToCopy == null)
                {
                    ...
                    mcParticleInitializationFunction?.Invoke(cParticle);
                }
        ...
        public int AddParticles(int numberOfParticlesToAdd)
            => AddParticles(numberOfParticlesToAdd, (Particle)null);
        ...
        public int AddParticles(int numberOfParticlesToAdd, ParticleEmitter emitterToLerp, float elapsedTimeInSeconds)
            ...
            => AddParticles(numberOfParticlesToAdd, new ParticleEmitterLerpInfo()
            {
        ...
        private void TryDraw(bool calledByDrawableGameComponent)
        {
            ...
            catch // (ObjectDisposedException) (WCS Edit) Eat everything...
            {
...
// Else we are running on Windows, so just draw the Quads
        ...
        /// pass in null.</param>
        public virtual bool AutoInitialize(GraphicsDevice cGraphicsDevice, ContentManager cContentManager, SpriteBatch cSpriteBatch)
            => true;
