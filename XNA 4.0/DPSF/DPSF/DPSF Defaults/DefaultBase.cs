...
namespace DPSF
{
    ...
    public class DPSFDefaultBaseParticle : DPSFParticle
    {
        ...
        /// </summary>
        public Color ColorAsPremultiplied => Color.FromNonPremultiplied(this.Color.ToVector4());
        ...
        /// </summary>
        public Color StartColorAsPremultiplied => Color.FromNonPremultiplied(this.StartColor.ToVector4());

        /// <summary>
        /// Get the End Color as a Premultiplied color (i.e. premultiplied alpha).
        /// </summary>
        public Color EndColorAsPremultiplied => Color.FromNonPremultiplied(this.EndColor.ToVector4());
    ...
    [Serializable]
#endif
    public abstract class DPSFDefaultBaseParticleSystem<Particle, Vertex>(Game cGame)
        : DPSF<Particle, Vertex>(cGame)
            ...
            where Vertex : struct, IDPSFParticleVertex
    {
        ...
        private readonly CInitialProperties mcInitialProperties = new();
        ...
        /// </summary>
        public List<DefaultParticleSystemMagnet> MagnetList = [];
        ...
        protected Vector3 CalculateForceMagnetShouldExertOnParticle(DefaultParticleSystemMagnet cMagnet, DPSFDefaultBaseParticle cParticle)
        {
            ...
            else if (cMagnet.MagnetType == DefaultParticleSystemMagnet.MagnetTypes.LineMagnet)
            {
                ...
                // Calculate the closest point on the Line to the Particle
                Vector3 sClosestPointOnLine = new()
                {
                    X = sPosition1.X + (fU * fD),
                    Y = sPosition1.Y + (fU * fE),
                    Z = sPosition1.Z + (fU * fF)
                };
            ...
            if (fDistanceFromMagnet >= cMagnet.MinDistance && fDistanceFromMagnet <= cMagnet.MaxDistance)
            {
                ...
                var fNormalizedForce = cMagnet.DistanceFunction switch
                {
                    DefaultParticleSystemMagnet.DistanceFunctions.Linear => MathHelper.Lerp(0, cMagnet.MaxForce, fLerpAmount),
                    DefaultParticleSystemMagnet.DistanceFunctions.Squared => MathHelper.Lerp(0, cMagnet.MaxForce, fLerpAmount * fLerpAmount),
                    DefaultParticleSystemMagnet.DistanceFunctions.Cubed => MathHelper.Lerp(0, cMagnet.MaxForce, fLerpAmount * fLerpAmount * fLerpAmount),
                    DefaultParticleSystemMagnet.DistanceFunctions.LinearInverse => MathHelper.Lerp(cMagnet.MaxForce, 0, fLerpAmount),
                    DefaultParticleSystemMagnet.DistanceFunctions.SquaredInverse => MathHelper.Lerp(cMagnet.MaxForce, 0, fLerpAmount * fLerpAmount),
                    DefaultParticleSystemMagnet.DistanceFunctions.CubedInverse => MathHelper.Lerp(cMagnet.MaxForce, 0, fLerpAmount * fLerpAmount * fLerpAmount),
                    _ => cMagnet.MaxForce,
                };
        ...
        protected static string GenerateRandomTextureFilePath(string textureFilePath, int min, int max)
            => textureFilePath + Random.Shared.Next(min, max);
        protected Texture2D GenerateRandomTexture(string textureFilePath, int min, int max)
            => ContentManager.Load<Texture2D>(GenerateRandomTextureFilePath(textureFilePath, min, max));
