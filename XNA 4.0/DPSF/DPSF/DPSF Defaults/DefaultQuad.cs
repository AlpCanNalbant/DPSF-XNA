...
namespace DPSF
{
    ...
    [Serializable]
#endif
    public abstract class DefaultQuadParticleSystem(Game cGame)
        : DPSFDefaultQuadParticleSystem<DefaultQuadParticle, DefaultQuadParticleVertex>(cGame)
    { }
    ...
    public class DefaultQuadParticle : DPSFDefaultBaseParticle
    {
        ...
        {
            get => Orientation3D.GetNormalDirection(Orientation);
        ...
        public Vector3 Up
        {
            get => Orientation3D.GetUpDirection(Orientation);
        ...
        {
            get => Orientation3D.GetRightDirection(Orientation);
    ...
    [Serializable]
#endif
    public abstract class DPSFDefaultQuadParticleSystem<Particle, Vertex>(Game cGame) : DPSFDefaultBaseParticleSystem<Particle, Vertex>(cGame)
        ...
        where Vertex : struct, IDPSFParticleVertex
    {
        ...
        private readonly CInitialPropertiesForQuad mcInitialProperties = new();
        ...
        protected override void SetEffectParameters()
        {
            ...
            if (this.Effect is not BasicEffect effect) return;
        ...
        protected void UpdateParticleRotationUsingRotationalVelocity(DefaultQuadParticle cParticle, float fElapsedTimeInSeconds)
        {
            ...
            if (cParticle.RotationalVelocity != Vector3.Zero)
            {
                ...
                Quaternion sRotation = new(cParticle.RotationalVelocity * (fElapsedTimeInSeconds * 0.5f), 0);
                cParticle.Orientation += cParticle.Orientation * sRotation;
            }
        }

        protected void UpdateParticleRotationRandomly(DefaultQuadParticle cParticle, float fElapsedTimeInSeconds)
        // {
        // cParticle.Orientation.Normalize();
            => cParticle.Orientation = Quaternion.CreateFromAxisAngle(Vector3.One, MathHelper.ToRadians(Random.Shared.Next(-360, 361)));
        // {
        //     if (fElapsedTimeInSeconds % 10 == 0)
        //     {
        //         cParticle.Orientation = Quaternion.CreateFromAxisAngle(Vector3.One, MathHelper.ToRadians(Random.Shared.Next(-360, 361)));
        //     }
        // }
        ...
        protected void UpdateParticlePositionRandomly(DefaultQuadParticle cParticle, float fElapsedTimeInSeconds)
            // {
            // cParticle.Orientation.Normalize();
            => cParticle.Position += new Vector3(MathHelper.ToRadians(Random.Shared.Next(-10, 10)));
        // }
        ...
        protected void UpdateParticleSystemToSortParticlesByDistanceFromCamera(float fElapsedTimeInSeconds)
        {
            ...
            List<Particle> cActiveParticleList = new(iNumberOfActiveParticles);
